...
def main(cfg):
    ...
    if cfg.env_name == 'LunarLander':
        env = create_env(config_file_name = 'lunarlander_continuos_medium', seed=10)
    else: 
        env = create_env(config_file_name = '', seed = 10)
    ...
    #agent = DDPG(state_shape, action_dim,max_action, lr = 3e-4, gamma=0.99, tau=0.005, batch_size=256, buffer_size=1e6)
    if cfg.agent_name == "pg_ac":
        agent = PG(state_shape[0], action_dim, cfg.lr, cfg.gamma)
    else: 
        agent = DDPG(state_shape, action_dim,max_action, lr = 3e-4, gamma=0.99, tau=0.005, batch_size=400, buffer_size=1e6) # with these parameters maybe early stopping at 300 episodes (since afterwards ep reward goes down)
