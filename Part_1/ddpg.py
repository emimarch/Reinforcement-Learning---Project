...
class DDPG(object):
    ...
    def _update(self,):
        ...
        states = batch.state # size torch[256,17]
        next_states = batch.next_state # size torch[256,17]
        rewards = batch.reward # size torch[256,1]
        actions = batch.action # size torch[256,6]
        not_done = batch.not_done # size torch[256,1]
    ...
    def get_action(self, observation, evaluation=False):
        ...
        if self.buffer_ptr < self.random_transition: # collect random trajectories for better exploration.
            ...
            if evaluation == False:
                noise = torch.normal(mean = 0, std=expl_noise, size=action.size())
                #print(noise)
